...
if (btnBack) btnBack.href = "map_mission.html";
...
function drawResult() {
    ...
    if (resultWin) {
        drawResultButton("Continuer", cx, cy + 50, 200, 44, "#c9a84c", "#1a0e00", () => {
            const base = `Roundforet.html?arena=${_arenaId}&player=${_playerId}&bg=${_bgFile}&from=map_mission&mapW=${_mapW}&mapH=${_mapH}`;
            const pos  = (_px && _py) ? `&px=${_px}&py=${_py}` : "";
            window.location.href = base + pos;
        });
    } else {
        drawResultButton("Retour à la carte", cx, cy + 50, 220, 44, "#333", "#fff", () => {
            window.location.href = "map_mission.html";
        });
    }
}
