...
// ================================
// Les positions x/y et métadonnées viennent de MISSION_CONFIG (map_data.js).
// On convertit l'objet en tableau pour l'itération.
const arenas = Object.entries(MISSION_CONFIG).map(([id, cfg]) => ({
    id:   Number(id),
    name: cfg.name,
    x:    cfg.x,
    y:    cfg.y,
...
canvas.addEventListener("click", (e) => {
    ...
    for (const arena of arenas) {
        ...
        const dy = my - arena.y;
        if (Math.sqrt(dx * dx + dy * dy) <= POINT_RADIUS + 6) {
            window.location.href = `character_select.html?arena=${arena.id}&from=map_mission`;
            return;
        }
    }
});
...
function drawTitle() {
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("✦  Choisissez votre mission  ✦", MAP_WIDTH / 2, 25);
}
